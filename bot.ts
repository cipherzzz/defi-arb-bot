...
const web3 = new Web3(new Web3.providers.WebsocketProvider(process.env.PROVIDER_URL));
...
const init = async () => {
  ...
  const updateEthPrice = async () => {
    const results = await kyber.methods
      .getExpectedRate('0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee', addresses.tokens.dai, 1)
      .call();
    ethPrice = web3.utils.toBN('1').mul(web3.utils.toBN(results.expectedRate)).div(ONE_WEI);
    console.log('ethPrice:', web3.utils.fromWei(ethPrice.toString()));
  };
  ...
  web3.eth
    ...
    .on('data', async (block) => {
      ...
      const ethFromKyber = AMOUNT_DAI_WEI.mul(web3.utils.toBN(amountsEth[0].expectedRate)).div(
        ONE_WEI
      );
      ...
      if (daiFromUniswap.gt(AMOUNT_DAI_WEI)) {
        ...
        console.log('tx cost', web3.utils.fromWei(txCost));
